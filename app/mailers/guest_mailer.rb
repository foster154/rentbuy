class GuestMailer < ActionMailer::Base
  ...
  def to_guest(guest)     # the initial email to the guest currently comes from the LENDER
    ...
    if @lender.email_settings.send_to_guest?
      if Rails.env.production?
      	mail to: @guest.email, bcc: "myrentwillbuy@gmail.com", reply_to: @lender.email_settings.reply_to_address, subject: @lender.email_settings.to_guest_subject, template_path: "guest_mailer/#{@guest.account.id}", template_name: "to_guest"
      else
  ...
  def to_agent(guest)   # new lead notification sent to the AGENT
    ...
    if @agent.email_settings.new_lead_notification?
      if Rails.env.production?
      	mail to: @agent.email_settings.send_leads_to_address, bcc: "myrentwillbuy@gmail.com", subject: @agent.email_settings.new_lead_subject, template_path: "guest_mailer/#{@guest.account.id}", template_name: "to_agent"
      else
